...
import { isShowChangeProfilePictureState, whiteBoardUserDataState } from "../../WhiteBoardStateManagement/Atom";
...
function ChangeProfilePicture() {
  ...
  const showCroppedImage = useCallback(async () => {
    try {
        ...
        localStorage.setItem('whiteboard_userProfilePicture' , croppedImage);

        setIsShowChangeProfilePicture(false);
        setChangeImageUrl("");
  ...
  return (
    <AnimatePresence>
      {isShowChangeProfilePicture && (
        <motion.div
          className="h-screen w-full fixed top-0 left-0 bg-black-opa80  flex justify-center items-center "
          style={{ zIndex: "100" }}
          animate={{ opacity: 1 }}
          initial={{ opacity: 0 }}
